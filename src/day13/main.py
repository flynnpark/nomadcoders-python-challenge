...
def scrape_page(term: str, url: str, soup: BeautifulSoup):
    if "stackoverflow" in url:
        return scrape_stackoverflow(soup)
    elif "weworkremotely" in url:
        return scrape_wework(soup)
    elif "remoteok" in url:
        return scrape_remoteok(soup)


def scrape_stackoverflow(soup: BeautifulSoup):
    jobs = []
...
def scrape_wework(soup: BeautifulSoup):
    jobs = []
...
def scrape_remoteok(soup: BeautifulSoup):
    jobs = []
...
app = Flask("DayThirteen", template_folder=f"{root_dir}/templates")
